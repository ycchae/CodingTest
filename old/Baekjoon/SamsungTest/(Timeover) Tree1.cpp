...
void tree_insert(int r, int c, int z){
    tree new_tree = {z, 0};
    list<tree>::iterator iter = lower_bound(map[r][c].trees.begin(), map[r][c].trees.end(), new_tree, comp);
...
void spring(){
    ...
    for(s = pos.begin(); s != pos.end(); ++s){
        ...
        for(list<tree>::iterator iter = cur.trees.begin(); iter != cur.trees.end(); ++iter){
        ...
        for(list<tree>::iterator iter = cur.trees.begin(); iter != cur.trees.end();){
            if((*iter).status == 1){
