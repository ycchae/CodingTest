...
void spring_summer(){
    ...
    for(s = pos.begin(); s != pos.end();){
        ...
        while(!cur.trees.empty()){
            if(cur.trees.front().status == 1){
                cur.nutri += cur.trees.front().age / 2;
            }else{
                new_trees.push_back(cur.trees.front());
            }
            cur.trees.pop_front();
        }
            ...
            if((*iter).age % 5 == 0){   // make child
                for(int i=0; i<8; ++i){
                    ...
                    if(nr < 1 || nr > N || nc < 1 || nc > N)
                        continue;
                    map[nr][nc].trees.push_front({1, 0});
